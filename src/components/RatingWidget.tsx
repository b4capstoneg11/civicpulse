...
export function RatingWidget({
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      <h2 className="mb-3 text-sm font-semibold text-slate-800">Was This Resolution Satisfactory?</h2>
      ...
      <fieldset className="mb-4">
        ...
        <div className="flex gap-2">
          {RATINGS.map((n) => (
            <label
              key={n}
              className={`flex h-11 w-11 cursor-pointer items-center justify-center rounded-full border text-sm font-semibold transition-colors [touch-action:manipulation] has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-teal-500 has-[:focus-visible]:ring-offset-2 ${
                rating >= n
